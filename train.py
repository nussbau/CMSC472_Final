...
def train(hyp, opt, device, callbacks):
    ...
    if RANK in {-1, 0}:
        ...
        if resume:  # If resuming runs from remote artifact
            weights, epochs, hyp, batch_size = opt.weights, 500, opt.hyp, opt.batch_size
    ...
    dip_model = DIP.IntegratedModel().to(device)
    dip_optim = torch.optim.Adam(dip_model.parameters(), lr=1e-4)
    dip_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(dip_optim, epochs, eta_min=1e-6, last_epoch=-1)
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                d_imgs = dip_model(imgs)
                pred = model(d_imgs)  # forward
            ...
            if ni - last_opt_step >= accumulate:
                ...
                scaler.unscale_(dip_optim)
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=10.0)  # clip gradients
                #torch.nn.utils.clip_grad_norm_(dip_model.parameters(), max_norm=100)
                scaler.step(optimizer)  # optimizer.step
                scaler.step(dip_optim)
                scaler.update()
                optimizer.zero_grad()
                dip_optim.zero_grad()
                dip_scheduler.step()
        ...
        # end epoch ----------------------------------------------------------------------------------------------------
    torch.save(dip_model, "DIP_Model.pth")
    # end training -----------------------------------------------------------------------------------------------------
    if RANK in {-1, 0}:
